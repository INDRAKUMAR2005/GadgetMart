...
const FloatingCart = () => {
  ...
  return (
    <button
      ...
      className="fixed bottom-12 right-12 z-50 bg-red-600 text-white px-8 py-6 rounded-[2rem] font-black shadow-2xl shadow-red-600/40 hover:scale-110 active:scale-95 transition-all flex items-center gap-4 group"
    >
      <div className="relative">
        <span className="text-2xl">🛒</span>
        <span className="absolute -top-2 -right-2 bg-orange-500 text-white text-[10px] w-5 h-5 flex items-center justify-center rounded-full border-2 border-red-600">{count}</span>
      </div>
...
export default function Home() {
  ...
  return (
    <div className='min-h-screen bg-[#0a0a0a] text-zinc-100 selection:bg-red-900/40 mesh-bg'>
      <div className='fixed inset-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-red-950/30 via-transparent to-transparent -z-10' />

      <FloatingCart />

      {/* Header / Nav */}
      <nav className="fixed top-0 left-0 right-0 z-40 bg-black/80 backdrop-blur-xl border-b border-zinc-800 px-6 py-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <div className="text-2xl font-black tracking-tighter cursor-pointer flex items-center gap-2" onClick={() => { setProductData(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }}>
            <div className="w-8 h-8 bg-gradient-to-br from-red-600 to-orange-500 rounded-lg flex items-center justify-center text-white text-lg font-black shadow-lg shadow-red-600/30">GM</div>
            <span className="text-white">Gadget<span className="text-orange-400">Mart</span></span>
          </div>
          <div className="flex items-center gap-8">
            <div className="hidden lg:flex items-center gap-6">

              <span
                className="text-xs font-black uppercase tracking-widest text-zinc-400 hover:text-orange-400 cursor-pointer transition-colors"
              ...
              <span
                className="text-xs font-black uppercase tracking-widest text-orange-400 hover:text-orange-300 cursor-pointer transition-colors"
            ...
            {user ? (
              <div className="flex items-center gap-6">
                <div className="hidden md:flex flex-col items-end">
                  <span className="font-bold text-[10px] text-orange-400 uppercase tracking-widest leading-none mb-1">{user.role || 'MEMBER'}</span>
                  <span className="font-black text-sm text-white leading-none truncate max-w-[150px]">{user.name || user.email}</span>
                </div>
                <button
                  onClick={handleLogout}
                  className="bg-zinc-800 text-zinc-300 px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-zinc-700 hover:text-white transition-all border border-zinc-700 shadow-sm"
                >
                  Sign Out
                </button>
              </div>
              ...
              <div className="flex items-center gap-6">
                <button
                  ...
                  className="bg-gradient-to-r from-red-600 to-orange-500 text-white px-8 py-3 rounded-2xl text-xs font-black uppercase tracking-widest hover:from-red-500 hover:to-orange-400 transition-all shadow-xl shadow-red-600/30 holographic"
                >
                  Sign In
                </button>
              </div>
      ...
      <header className='pt-48 pb-16 px-6 flex flex-col items-center text-center'>
        <div className='inline-flex items-center gap-2 px-4 py-1.5 mb-8 rounded-full bg-red-950/60 border border-red-800/50 text-orange-400 text-sm font-black animate-fade-in tracking-tight uppercase'>
        ...
        <h1 className='text-6xl md:text-[8rem] font-black mb-8 tracking-tighter text-white leading-[0.85]'>
          Tech Found. <br />
          <span className='gradient-text'>Prices Compared.</span>
        </h1>
        <p className='text-zinc-400 text-xl md:text-2xl max-w-3xl mx-auto font-medium leading-relaxed pb-8'>
          Real-time price intelligence across <span className="text-white font-black">Global Retailers</span>. <br />
          Always get the absolute <span className="text-orange-400 font-bold">Lowest Price</span> guaranteed.
        </p>
      ...
      <main className='max-w-7xl mx-auto px-6 pb-32'>
        <div className="relative group max-w-2xl mx-auto">
          <form onSubmit={handleSearch} className="relative z-10 flex items-center bg-zinc-900/90 backdrop-blur-2xl rounded-[2rem] p-2 premium-shadow border border-zinc-700/60 group-focus-within:border-red-500/60 transition-all">
            <input
              type="text"
              placeholder="Search for iPhones, MacBooks, or specialized gear..."
              className="flex-grow px-8 py-4 bg-transparent text-lg font-semibold text-white focus:outline-none placeholder:text-zinc-600"
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              onFocus={() => searchQuery.length > 1 && setShowSuggestions(true)}
            />
            <button type="submit" className="bg-gradient-to-r from-red-600 to-orange-500 text-white p-4 rounded-2xl hover:from-red-500 hover:to-orange-400 hover:scale-105 transition-all shadow-lg shadow-red-600/30 active:scale-95">
          ...
          {showSuggestions && suggestions.length > 0 && (
            <div className="absolute top-full left-0 right-0 mt-4 bg-zinc-900/95 backdrop-blur-3xl rounded-[2.5rem] border border-zinc-700 p-6 shadow-2xl shadow-black/50 z-50 animate-in slide-in-from-top-4 duration-300">
              <h4 className="text-[10px] font-black uppercase tracking-widest text-zinc-500 mb-4 ml-2">Quick Matches</h4>
              <div className="space-y-4">
                {suggestions.map((p, idx) => (
                  <div
                    key={idx}
                    className="flex items-center gap-4 p-4 rounded-2xl hover:bg-red-950/50 cursor-pointer transition-all group/item"
                    onClick={() => {
                  ...
                  >
                    <div className="w-12 h-12 bg-zinc-800 rounded-xl overflow-hidden border border-zinc-700 flex items-center justify-center text-xl grayscale group-hover/item:grayscale-0 transition-all">
                      <img src={p.image} className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-grow">
                      <p className="font-bold text-white">{p.name}</p>
                      <p className="text-xs font-bold text-orange-400">₹{p.price.toLocaleString()}</p>
                    </div>
                    <svg className="w-5 h-5 text-zinc-600 group-hover/item:text-orange-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M17 8l4 4m0 0l-4 4m4-4H3" /></svg>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="max-w-4xl mx-auto mt-12 p-8 bg-zinc-950 text-white rounded-[3rem] shadow-2xl border border-zinc-800 relative overflow-hidden group">
          <div className="absolute top-0 right-0 w-64 h-64 bg-red-600/15 blur-[100px] -z-10 group-hover:bg-red-600/25 transition-all duration-700" />
          <div className="flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
            <div className="w-16 h-16 bg-gradient-to-br from-red-600 to-orange-500 rounded-2xl flex items-center justify-center text-3xl shadow-lg shadow-red-600/20 shrink-0 animate-pulse">⚠️</div>
            <div className="space-y-4">
              <h4 className="text-xs font-black uppercase tracking-[0.3em] text-orange-400">Project Integrity & Transparency</h4>
              ...
              <div className="p-4 bg-white/5 rounded-2xl border border-white/10">
                <p className="text-sm font-bold text-white">
                  📍 <span className="text-orange-400 uppercase tracking-widest text-[10px] mr-2">Highlight</span>
                  You can check the exact details and live pricing of any product by using the <span className="text-orange-400 underline decoration-2 underline-offset-4">Grab Now</span> feature.
                </p>
              </div>
            </div>
          </div>
        ...
        {productData ? (
          <div className='space-y-16 animate-in fade-in slide-in-from-bottom-5 duration-500'>
            <div className='flex flex-col md:flex-row md:items-end justify-between border-b border-zinc-800 pb-12 gap-6'>
              <div className="flex items-center gap-8">
                <div className="w-32 h-32 rounded-3xl overflow-hidden border border-zinc-700 shadow-lg shrink-0">
                ...
                <div>
                  <h2 className='text-5xl font-black tracking-tighter text-white'>{productData.name}</h2>
                  <p className='text-zinc-400 text-lg font-medium mb-2'>{productData.brand} • {productData.category}</p>
                  <p className='text-orange-400/80 text-sm font-bold uppercase tracking-widest'>{productData.description}</p>
                </div>
              </div>
              <div className='text-right'>
                <span className="text-sm font-black text-zinc-500 uppercase tracking-widest">Global Best Price</span>
          ...
          <div className="space-y-32">
            ...
            <div id="extreme-deals">
              <div className='flex items-center justify-between mb-12'>
                <div>
                  <h2 className='text-4xl font-black text-white tracking-tight'>Extreme Deals</h2>
                  <p className="text-zinc-500 font-medium">Hand-picked lowest prices found today.</p>
                </div>
                <div className='h-px flex-grow mx-8 bg-zinc-800 hidden md:block'></div>
              </div>
              <div className='grid grid-cols-1 md:grid-cols-2 gap-8'>
                {HOT_OFFERS.slice(0, 10).map((gadget, i) => (
                  <div key={i} className='flex items-center gap-6 glass-card p-6 rounded-[2.5rem] border border-zinc-800 hover:border-red-600/60 hover:shadow-2xl hover:shadow-red-900/20 transition-all cursor-pointer group holographic'
                    onClick={() => {
                      if (!user) {
                        router.push('/login');
                        return;
                  ...
                  >
                    <div className='w-40 h-40 rounded-[2rem] overflow-hidden border border-zinc-800 shrink-0'>
                    ...
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <h3 className='text-xl font-black text-white'>{gadget.name}</h3>
                      </div>
                      <button
                        onClick={(e) => {
                          e.stopPropagation();
                        ...
                        className="w-10 h-10 rounded-full bg-zinc-800 flex items-center justify-center hover:bg-red-950 hover:text-red-400 transition-all group/save"
                      >
                    ...
                    <div className="flex items-baseline gap-2">
                      <p className='text-3xl font-black text-white'>₹{gadget.price.toLocaleString()}</p>
                      <span className="text-zinc-600 text-sm line-through font-bold">₹{(gadget.price * 1.4).toLocaleString()}</span>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div id="fresh-arrivals">
              <div className='flex items-center justify-between mb-12'>
                <div>
                  <h2 className='text-4xl font-black text-white tracking-tight'>Fresh Arrivals</h2>
              ...
              <div className='grid grid-cols-1 md:grid-cols-3 gap-8'>
                {NEW_LAUNCHES.map((gadget, i) => (
                  <div key={i} className='glass-card p-6 rounded-[2.5rem] border border-zinc-800 hover:border-orange-600/40 hover:shadow-2xl hover:shadow-orange-900/10 transition-all cursor-pointer group holographic'
                    onClick={() => {
                      if (!user) {
                        router.push('/login');
                        return;
                  ...
                  >
                    <div className='aspect-square rounded-[2rem] overflow-hidden mb-8 border border-zinc-800 group-hover:scale-105 transition-transform'>
                      <img src={gadget.image} alt={gadget.name} className="w-full h-full object-cover" />
                    </div>
                    <div className="px-2">
                      <div className="flex justify-between items-start mb-2">
                        <div className="flex-grow">
                          <h3 className='text-2xl font-black text-white leading-tight'>{gadget.name}</h3>
                          <p className='text-zinc-500 font-bold text-sm uppercase tracking-widest mt-1'>{gadget.brand}</p>
                        </div>
                      ...
                      <div className="flex justify-between items-center bg-zinc-800/60 rounded-2xl p-4 border border-zinc-700 mt-4">
                        <p className='text-xl font-black text-white'>₹{gadget.price.toLocaleString()}</p>
                        <span className="text-orange-400 font-black text-xs uppercase tracking-widest">Compare →</span>
                      </div>
                    </div>
                  </div>
                ))}
      ...
      {/* Creative Footer */}
      <footer className="bg-black text-white pt-32 pb-12 px-6 overflow-hidden relative border-t border-zinc-900">
        <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-64 bg-red-700 blur-[120px] opacity-15" />
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-24 mb-24">
            ...
            <div className="flex flex-col justify-end lg:items-end">
              <div className="mb-12">
                <span className="text-orange-500 font-black text-xs uppercase tracking-[0.3em] block mb-4">Masterminds Behind GadgetMart</span>
              ...
              <div className="flex flex-wrap gap-12 lg:justify-end text-zinc-400 text-[10px] font-black uppercase tracking-widest">
                <div className="group relative">
                  <span className="hover:text-white transition-colors cursor-pointer underline decoration-orange-600">Documentation</span>
                  <div className="absolute bottom-full mb-4 hidden group-hover:block bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-2xl w-72 text-left animate-in fade-in zoom-in duration-300">
                    <p className="text-orange-400 mb-2">Technical Core</p>
                ...
                <div className="group relative">
                  <span className="hover:text-white transition-colors cursor-pointer underline decoration-orange-600">API Access</span>
                  <div className="absolute bottom-full mb-4 hidden group-hover:block bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-2xl w-72 text-left animate-in fade-in zoom-in duration-300">
                    <p className="text-orange-400 mb-2">Developer Gateway</p>
                ...
                <div className="group relative">
                  <span className="hover:text-white transition-colors cursor-pointer underline decoration-orange-600">Infrastructure</span>
                  <div className="absolute bottom-full mb-4 hidden group-hover:block bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-2xl w-72 text-left animate-in fade-in zoom-in duration-300">
                    <p className="text-orange-400 mb-2">Cloud Foundation</p>
                ...
                <div className="group relative">
                  <span className="hover:text-white transition-colors cursor-pointer underline decoration-orange-600">Terms</span>
                  <div className="absolute bottom-full mb-4 hidden group-hover:block bg-zinc-900 border border-zinc-800 p-6 rounded-3xl shadow-2xl w-72 text-left animate-in fade-in zoom-in duration-300">
                    <p className="text-orange-400 mb-2">Usage Agreement</p>
      ...
      {/* Premium Daily Notification Toast */}
      {
        showNotification && (
          <div className="fixed top-24 right-8 z-50 animate-in slide-in-from-right-10 duration-700">
            <div className="bg-zinc-950 border border-zinc-800 p-8 rounded-[2.5rem] shadow-2xl shadow-red-900/30 max-w-sm relative overflow-hidden group">
              <div className="absolute top-0 right-0 w-32 h-32 bg-red-600/15 blur-[60px] -z-10 group-hover:bg-red-600/25 transition-all" />
              ...
              <div className="flex items-center gap-4 mb-4">
                <div className="w-12 h-12 bg-gradient-to-br from-red-600 to-orange-500 rounded-2xl flex items-center justify-center text-2xl">⚡</div>
                <div>
                  <span className="text-orange-400 font-black text-[10px] uppercase tracking-widest block">Daily Flash Sale</span>
                  <h4 className="text-white font-black text-xl tracking-tighter">Extreme Tech Drop</h4>
                </div>
              </div>
              <p className="text-zinc-400 text-sm leading-relaxed mb-6">
                Grab the <span className="text-white font-bold">AirPods Pro Max (2026 Edition)</span> at a record-low price of <span className="text-orange-400 font-black">₹34,999</span> for the next 4 hours!
              </p>
              <button
                onClick={() => { setShowNotification(false); document.getElementById('extreme-deals')?.scrollIntoView({ behavior: 'smooth' }); }}
                className="w-full bg-gradient-to-r from-red-600 to-orange-500 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest hover:from-red-500 hover:to-orange-400 transition-all shadow-xl shadow-red-600/30"
              >
                Claim Deal Now →
              </button>
            </div>
